...
export function calculateEMI(principal, monthlyRate, emi) {
  if (typeof principal !== 'number' || typeof monthlyRate !== 'number' || typeof emi !== 'number' || principal <= 0 || monthlyRate <= 0 || emi <= 0 || emi <= principal * monthlyRate) {
    return { months: -1, totalPaid: -1, totalInterest: -1 };
  }
  let totalPaid = 0;
  let totalInterest = 0;
  ...
  return { months: months, totalPaid: totalPaid, totalInterest: parseFloat(totalInterest.toFixed(2))};
}
